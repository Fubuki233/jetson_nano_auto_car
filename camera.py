...
img5: 动态阈值算法
'''
img = cv2.imread('./photos/color.jpg')
...
i0 = np.vstack([img, img1])
i1 = np.vstack([ img2, img3])
i2 = np.vstack([ img4, img5])
# cv2.imwrite("./dataset/"+str(i)+'.JPG',img_stack)
    #img1=np.array('img_stack'+str(i))
cv2.imwrite('str(1).jpg',i0)
cv2.imwrite("str(2).jpg",i1)
cv2.imwrite("str(3).jpg",i2)
cv2.waitKey(0)